...
var EvaluationController=function(Model){
	return {
		...
		save:function(data,newEval,callback){
			Model.Evaluation.findOneAndUpdate(data, {$set:{eval:newEval.eval}}, {upsert:true}, function(err, data){
		        if(err){
		            console.error(err);
		            callback({error: 1});
		            return;
		...
		get:function(postID,fromMember,callback){
			Model.Evaluation.find({postID:postID,fromMember:fromMember}, function(err,data){
		        if(err){
		            console.error(err);
		            callback({error: 1});
		            return;
